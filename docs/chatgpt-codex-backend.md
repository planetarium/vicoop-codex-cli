...
| 필드 | 타입 | 우리 동작 | 비고 |
|---|---|---|---|
| `model` | string | `-m` (**필수, 기본값 없음**) | 누락 시 거절하고 현재 사용 가능 목록을 안내. 백엔드 모델 목록에 있는 슬러그만 허용 |
...
`-m`은 필수다. 생략하면 exit 2로 거절하며 현재 사용 가능한 모델 목록을 안내한다(`vicoop-codex models`로도 조회).

```bash
# -m + (기본 instructions)
vicoop-codex prompt -m gpt-5.5 "say hi"

# -m + -i + -r
vicoop-codex prompt -m gpt-5.5 -i "Reply in Korean only." -r high "지구의 둘레는?"
```
...
- `gpt-5.4`
- `gpt-5.4-mini`
- `gpt-5.3-codex` (이후 백엔드 목록에서 제외됨 — CLI 기본값도 폐지)
- `gpt-5.2`
- `codex-auto-review`

모델은 더 이상 CLI에 하드코딩된 기본값이 없다. 어떤 모델이든 `-m`(또는 요청 body의 `model`)으로 명시해야 하며, 누락 시 거절된다 — `src/client/responses.ts`의 `RunRequest.model`은 필수이고 `buildBody`는 `req.model`을 그대로 전송한다. 가용 슬러그가 바뀌어도 코드는 손댈 필요가 없고(런타임에 `/codex/models`로 안내), 필요하면 `src/index.ts`의 `-m` 옵션 설명 문구 정도만 갱신하면 된다.
...
| 가용 모델 | `/codex/models`로 조회 — `gpt-5.5`, `gpt-5.4` 등 한정된 풀 (시점에 따라 변동) | `/v1/models` — `gpt-5.5`, `gpt-5.4`, `o4-mini`, 등 공개된 전체 카탈로그 |
...
3. `finish_reason` (`determineFinishReason`):
   - tool_call 있음 → `"tool_calls"`
   - `response.status === "incomplete"` && `incomplete_details.reason === "max_output_tokens"` → `"length"`
   - `response.status === "incomplete"` && `incomplete_details.reason === "content_filter"` → `"content_filter"`
   - 그 외 → `"stop"`
4. `usage` 키 이름 변환:
...
| `response.output_item.done` (function_call) | role 청크가 아직 안 갔으면 `delta:{role:"assistant",content:null}` 먼저, 이어서 `data: {"choices":[{"delta":{"tool_calls":[{"index":N,"id":"call_xxx","type":"function","function":{"name":"...","arguments":"..."}}]}}], ...}` |
| `response.completed` | `data: {"choices":[{"delta":{},"finish_reason":"stop|length|content_filter|tool_calls"}], "usage":{...}?}` + `data: [DONE]`. `usage`는 `stream_options.include_usage: false`이면 생략 |
...
- `seed`, `logprobs`, `n`, `frequency_penalty`, `presence_penalty`, `max_tokens` 미지원 (백엔드가 거절). 필요 시 본문에서 제거.
- 멀티모달 입력 중 텍스트만 추출. 이미지/오디오 파트는 무시되며 `dropped` 로그에 `messages[].content[] non-text parts: <type>` 형태로 기록됨.
